    ...
    Y.namespace('LogiXML').DraggableElement = Y.Base.create('DraggableElement', Y.Base, [], {
        ...
        attachDrag: function () {
            ...
            if (this.proxyMode == "element") {
                dragPrms.dragNode = Y.one('#' + this.proxyId);
            }

            if (this.handlerId && this.handlerId.length > 0) {
                dragPrms.handles = this.configNode.all('#' + this.handlerId);
            }
        ...
        onDragEnd: function (e) {
            ...
            if (this.dropStack.length > 1) {
                Y.one('#' + this.dropStack[this.dropStack.length - 1]).getData('rdDroppableContainer').checkPlaceholder();
            }
            this.dropStack = [];
        },
        ...
        onDragEnter: function (e) {
            ...
            if (this.dropStack.length > 1) {
                Y.one('#' + this.dropStack[this.dropStack.length - 2]).getData('rdDroppableContainer').removeGhost();
            }
            var hoverNode = this.getHoverNode(e);
            Y.one('#' + dropId).getData('rdDroppableContainer').showGhost(e.drag, hoverNode);
        ...
        onDragOver: function (e) {
            ...
            var hoverNode = this.getHoverNode(e);
            Y.one('#' + dropId).getData('rdDroppableContainer').moveGhost(e.drag, hoverNode);
        },
        ...
        destructor: function () {
            ...
            if (this.handlerId && this.handlerId.length > 0 && this.handlers.drag) {
                this.handlers.drag.removeHandle = this.configNode.one('#' + this.handlerId);
            }
