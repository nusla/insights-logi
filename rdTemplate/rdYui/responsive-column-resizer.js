YUI.add('responsiveColumnResizer', function (Y) {
    ...
    Y.namespace('LogiXML').ResponsiveColumnResizer = Y.Base.create('ResponsiveColumnResizer', Y.Base, [], {
        ...
        resizeColumnContent: function (column, colspan) {
            ...
            Y.LogiXML.ChartCanvas.resizeToWidth(column.get('offsetWidth') - paddingConstant, '#' + columnId + ' > .rd-editor-visualization');

            nestedColumns = Y.one('#' + columnId).all('.column-with-droppable');
            Y.each(nestedColumns, function (nestedColumn) {
                Y.LogiXML.ChartCanvas.resizeToWidth(nestedColumn.get('offsetWidth') - paddingConstant, '#' + nestedColumn.getAttribute('id') + ' > .rd-editor-visualization');
            });
        },
